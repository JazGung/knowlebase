...
| started_at | TIMESTAMP | N | 开始时间（数据库默认当前时间戳，构建/启用时由服务层显式更新） |
...
| relation_type | String(20) | N | 关联类型。存储映射：INITIAL→"initial", INCREMENTAL→"incremental" |
...
| 字段 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| files | List[FileCheckItem] | M | 文件检查项列表，每项含 filename (str) 和 hash (str, 32位MD5)，用于上传前检测重复文件 |
...
| 字段 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| file | File | M | PDF/Word 文件，≤100MB |
| hash | str | M | 前端计算的 MD5 哈希值，后端上传后重新计算比对，不一致则抛出 `FILE_HASH_MISMATCH` |
| title | str | O | 文档标题，不传则取原始文件名 |
