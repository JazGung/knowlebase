...
| GET | /build/detail | view | 处理详情查询 | REQ #5 |
...
所有模型调用通过 Higress AI 网关统一路由。应用仅配置网关地址，逻辑名由代码常量指定。
...
| `embedding` | 数据入库 | /model/embedding（本地 sentence-transformers） |

provider、api_key、temperature 等细节由 Higress 侧配置，应用不感知。
...
3) 通过 Higress 发送解析请求（model="parsing"），携带 file_content、file_format、file_name
4) Higress 路由到 /model/parsing（本地 pdfplumber/docx）或在线解析服务，接收 ParseResult
5) Higress 调用失败或解析失败则 UPDATE processing_stage_result（status='failed', error_message），记录 ERROR 日志，抛出 `PARSING_FAILED`
6) 记录阶段完成：UPDATE processing_stage_result（status='succeeded', duration_ms），发布 Processing_Stage_Completed_Event
...
3) 从 MinIO 读取图片文件，通过 Higress 发送图片描述请求（model="image-desc"）调用视觉模型生成描述文字
...
3. **HyDE 生成也需要独立调用**：向量库写入前需单独调用 LLM 生成假设问题，增加 API 调用次数
4. **图片处理孤立**：图片与文本分离，无法关联上下文

将分块、指代消解、HyDE 问题生成、图谱关系提取合并为一次 LLM 调用（每窗口一次），可以：减少 API 调用次数；模型在理解上下文语境后同时输出，质量更高；指代消解结果可以直接基于分块的语义边界，更精准。

选用 LangChain 而非直接调用 API：`init_chat_model` 统一接口，后续切换 LLM 只需改配置；`JsonOutputParser` 保证输出结构化，避免正则解析 JSON 的脆弱性。
...
2) 按三级标题（H1/H2/H3）将文本划分为独立分块单位，一个分块内不能包含多个同级标题。H4 及以下不作为单独分块单位，多个可合并到同一分块
3) 判断 section 文本是否 ≤ max_chars：
   - 是：直接作为一个 chunk，只做指代替换、假设提问、关系提取
   - 否：进入窗口滑动
4) 执行窗口滑动：
   - **窗口1（首个窗口）**：Context 为空，Current Window 为前 window_size 字符，前溯到句子/图片/表格行边界
   - **窗口2+**：Context 包含：① 上一 chunk `processed_text` 末尾 ≥ overlap 长度的完整句子/图片/表格行；② 上一分块提取的实体列表。Current Window 从上一窗口结束处起，window_size 字符，前溯到边界
   - **输出处理**：丢弃最后一个 chunk（边界截断风险），最后一个窗口除外。每个 chunk 的 `processed_text` 在开头添加重叠部分，`original_text` 不加
5) 对每个窗口通过 Higress 发送分块请求（model="chunking"），LLM Pipeline 单次调用内按序执行 4 步：
   - **语义分块**：如果文本 > max_chars，按语义分成多个 sub-chunk（每块 ≥ min_chars 且 ≤ max_chars）
   - **指代替换**：结合 Context 做指代消解
   - **假设提问**：每块生成 2-5 个 HyDE 问题
   - **关系提取**：提取 `(source, relationship, target)` 三元组
6) LLM 返回 JSON 映射为 ChunkResult：

| LLM 字段 | ChunkResult 字段 | 说明 |
| :--- | :--- | :--- |
| `original_text` | `original_text` | 原始分块文本（不修改任何字符） |
| `processed_text` | `processed_text` | 指代消解后的文本，开头添加重叠前缀 |
| `questions` | `hypothetical_questions` | 2-5 个假设问题（HyDE） |
| `triplets` | `relations` | `(source, relationship, target)` 三元组 |

7) 分块后还原：
   - 还原 H4 及以下标题到所属 chunk
   - 合并过短的分块（< min_chars）
   - 图片占位符格式：`[IMAGE: path=images/xxx.png|caption=描述]`，前端可通过 `/build/image/preview?path=xxx` 获取 MinIO 预签名 URL 渲染为 `<img>`；`original_text` 保留此占位符，`processed_text` 保留完整图片描述文字 + 指代消解用于向量入库
...
- Neo4j 层使用 `MERGE` 处理节点去重，LLM 输出不关心去重
...
6) Milvus 向量写入：对每个 chunk，拼接文本 → 通过 Higress 发送嵌入请求（model="embedding"） → 接收向量 → insert {chunk_id, document_id, vector, enabled}
7) Neo4j 图谱写入：对每个三元组：
    ...
    "tabs": [
      {
        ...
        "stages": [
          ...
          { "name": "stored", "status": "pending", "duration_ms": null }
        ],
        "error_message": null
      }
    ]
