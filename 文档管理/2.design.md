...
| admin.document.domain.DocumentProcessingHistory | Y | 处理历史领域对象 |
| admin.document.domain.ProcessingStageResult | Y | 阶段结果领域对象 |
| admin.document.domain.DocumentChunk | Y | 文档分块领域对象 |
| admin.document.domain.ChunkResult | N | 分块结果数据类 |
| admin.document.domain.ParseResult | N | 解析结果数据类 |
...
| status | status | 文档状态（enabled/disabled） |
...
| status | String(20) | N | 枚举：enabled/disabled |
...
| source_type | String(50) | Y | 来源类型：upload/api/crawl/import |
| rebuild_id | String(36) | Y | 关联的重建记录ID |
...
| status | String(20) | N | pending/processing/succeeded/failed |
...
| status | String(20) | N | running/succeeded/failed |
...
| 方法 | 入口 | 流程 |
| :--- | :--- | :--- |
| stage_completed(processing_id, stage_name, duration_ms) | DocumentProcessingEvent.stage_completed() | 创建 StageCompletedEvent(status="succeeded")，调用 ProcessingEventBus.publish() |
| stage_failed(processing_id, stage_name, duration_ms, error_message) | DocumentProcessingEvent.stage_failed() | 创建 StageCompletedEvent(status="failed")，调用 ProcessingEventBus.publish() |
...
| GET | /build/processing/status/{processing_id} | status | 处理状态查询 | REQ #9 |
| GET | /build/processing/stage/{processing_id}/{stage_name} | stage_result | 阶段结果详情 | REQ #9 |
| GET | /build/processing/view | view | 处理过程视图 | REQ #10 |
...
6) 开启数据库事务：INSERT document 记录（status='enabled', attempt_no=1）
7) 提交事务，提交失败则回滚并尝试从 MinIO 删除已写入的孤立文件，抛出 `DOCUMENT_SAVE_FAILED`
8) 后台异步触发文档处理任务，触发失败记录 ERROR 日志不影响主流程。处理开始时由文档检查阶段（4.10.5）创建 document_processing_history 记录
...
3) 尝试从 MinIO 读取文件头部验证可读性（stat_object + 文件大小 > 0），不可读则 INSERT document_processing_history（status='failed'），写入 error_message='文档文件不可读'，终止处理
